...
export class ReservationsService {
  ...
  /**
   * PHASE 1 — simplified version of the "transactional brain".
   *
   * Consistency guaranteed at the database level: we use a
   * pessimistic lock (`SELECT ... FOR UPDATE`) on the event row within
   * a transaction, so two concurrent requests can never read the same
   * `availableTickets` value and oversell tickets.
   *
